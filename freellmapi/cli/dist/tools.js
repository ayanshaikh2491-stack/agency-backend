...
function primaryModel(models, requestedId) {
    ...
    if (requestedId)
        return models.find(model => model.id === requestedId) ?? { id: requestedId };
...
function goose(ctx) {
    ...
    return {
        files: [
            {
                path: path.join(ctx.homeDir, '.config', 'goose', 'custom_providers', 'freellmapi.json'),
                format: 'json',
                value: {
                    name: 'freellmapi',
                    engine: 'openai',
...
function mimo(ctx) {
    ...
    const configDir = process.env.MIMOCODE_HOME?.trim()
        ...
        : path.join(process.env.XDG_CONFIG_HOME?.trim() || path.join(ctx.homeDir, '.config'), 'mimocode');
    ...
    const roster = catalogModels(ctx.models);
    const modelEntries = Object.fromEntries([model, ...roster.filter(entry => entry.id !== model.id)].map(entry => [entry.id, {
            name: entry.name ?? entry.id,
...
export const tools = metadata.map(([id, name, category, configType, protocol, baseUrlSupport, command, docsUrl, generate,]) => ({
    id,
    name,
    category,
    configType,
