...
function mergeJson(existing, value) {
    ...
    const applyPatch = (base, patch, parentPath = []) => {
        for (const [key, next] of Object.entries(patch)) {
            ...
            rendered = applyEdits(rendered, modify(rendered, propertyPath, next, { formattingOptions }));
        }
    };
...
// ours changed.
function mergeYamlDocument(existing, value) {
    if (!existing.trim()) {
    ...
    const applyPatch = (patch, parentPath = []) => {
        for (const [key, next] of Object.entries(patch)) {
            const propertyPath = [...parentPath, key];
            if (next === undefined) {
...
function markerPattern() {
    return new RegExp(`^[ \\t]*(?:#|//) freellmapi:start[\\s\\S]*?^[ \\t]*(?:#|//) freellmapi:end\\s*`, 'm');
}
function mergeMarked(existing, generated) {
...
function generatedYamlKeys(generated) {
    return new Set(generated.split(/\r?\n/)
        .map(topLevelYamlKey)
        .filter((key) => Boolean(key)));
}
function removeYamlKeys(existing, keys) {
    ...
    for (let index = 0; index < lines.length;) {
        ...
        index += 1;
        while (index < lines.length && !topLevelYamlKey(lines[index]))
            index += 1;
    }
    return output.join('\n').replace(/\n{3,}/g, '\n\n').trimEnd();
}
function continueModelLines(generated) {
    const lines = generated.split(/\r?\n/);
    const modelsIndex = lines.findIndex(line => line === 'models:');
    const endIndex = lines.findIndex((line, index) => index > modelsIndex && line === '# freellmapi:end');
    return lines.slice(modelsIndex + 1, endIndex < 0 ? undefined : endIndex);
}
function mergeContinueYaml(existing, generated) {
    ...
    while (modelsEnd < lines.length && !topLevelYamlKey(lines[modelsEnd]))
        modelsEnd += 1;
    const entryStart = lines.findIndex((line, index) => index > modelsIndex
        && index < modelsEnd
        && /^\s{2}-\s+name:\s+["']?FreeLLMAPI["']?\s*$/.test(line));
    if (entryStart >= 0) {
        let entryEnd = entryStart + 1;
        while (entryEnd < modelsEnd
    ...
    if (missingHeader.length)
        lines = [...missingHeader, ...lines];
    return `${lines.join('\n').replace(/\n+$/, '')}\n`;
}
function mergeYaml(existing, generated) {
    ...
    if (!existing.trim())
        return generated;
    if (/^# freellmapi:start$/m.test(existing))
        return mergeMarked(existing, generated);
...
function tomlRootKey(line) {
    return line.match(/^([A-Za-z0-9_.-]+)\s*=/)?.[1];
}
function isTomlMarkerLine(line) {
...
function mergeToml(existing, generated) {
    if (!existing.trim())
        return generated;
    ...
    for (const line of existing.split(/\r?\n/)) {
        if (isTomlMarkerLine(line))
            continue;
        const table = tomlTableHeader(line);
        if (table)
            currentTable = table;
        if (currentTable && tableNames.has(currentTable))
            continue;
        if (!currentTable && rootKeys.has(tomlRootKey(line) ?? ''))
            continue;
        kept.push(line);
    }
...
function mergeEnv(existing, generated) {
    const replacements = new Map(generated.split(/\r?\n/)
...
export function renderFile(file, existing = '') {
    if (file.format === 'json')
        return mergeJson(existing, file.value ?? {});
    if (file.format === 'env')
        return mergeEnv(existing, file.content ?? '');
    if (file.format === 'yaml') {
        return file.value
...
function planGeneratedFile(file) {
...
function commitPlannedWrite(plan) {
...
export function applyGeneratedFiles(files, dryRun) {
    const plans = files.map(planGeneratedFile);
    if (dryRun)
        return plans.map(({ file: _file, exists: _exists, ...result }) => result);
    return plans.map(commitPlannedWrite);
}
export function applyGeneratedFile(file, dryRun) {
    return applyGeneratedFiles([file], dryRun)[0];
}
export function printDryRunDiff(result) {
