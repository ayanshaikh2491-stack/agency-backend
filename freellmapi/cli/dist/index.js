...
import { DOCTOR_TOOLS, diagnose, exitCodeFor, formatReport } from './doctor.js';
...
function validateProfile(profile) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(profile)
        || profile === '.'
        || profile === '..') {
        throw new Error('--profile must use only letters, numbers, dots, underscores, or hyphens');
    }
    return profile;
}
...
export function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (flag === '--url' || flag === '--api-key' || flag === '--profile'
...
async function promptForKey() {
    if (!process.stdin.isTTY) {
        throw new Error('No API key supplied. Pass --api-key or set FREELLMAPI_API_KEY.');
    }
    let muted = false;
...
async function catalog(url, apiKey, availableOnly = true) {
    let response;
    try {
        ...
        throw new Error(`Could not reach the FreeLLMAPI gateway at ${rootUrl(url)} (${reason}). `
    ...
    if (!models.length)
        throw new Error('The live catalog returned no models');
    return models;
}
/**
 ...
 */
export async function catalogs(url, apiKey) {
 ...
 */
export function resolvePinnedModel(requested, rosters, warn = message => process.stderr.write(message)) {
    if (!requested)
        return undefined;
    ...
    if (rosters.degradedReason) {
        warn(`freellmapi: could not fetch the unfiltered model catalog (${rosters.degradedReason}). `
    ...
    if (resolved.unavailable) {
        warn(`freellmapi: '${resolved.id}' is registered but not currently available `
...
async function setup(command, options) {
    ...
    if (generation.files.length === 0) {
        for (const note of generation.notes)
...
export function claudeLaunchEnv(options, apiKey, models, baseEnv = process.env, homeDir = os.homedir(), fullCatalog = models) {
    ...
    if (options.profile !== 'default') {
        ...
        if (!fs.existsSync(directory)) {
            throw new Error(`Claude profile '${options.profile}' has no configuration directory at ${directory}. `
...
async function launchClaude(options) {
    ...
    const env = claudeLaunchEnv(options, apiKey, rosters.available, process.env, os.homedir(), rosters.full);
    return runChild('claude', [], env);
}
export function codexArgs(url, selected) {
    return [
...
async function launchCodex(options) {
    const apiKey = options.apiKey ?? await promptForKey();
    const rosters = await catalogs(options.url, apiKey);
    const selected = (resolvePinnedModel(options.model, rosters)
        ?? resolveLaunchModel(undefined, rosters.available, rosters.full)).id;
